...
class CGMagicClassTrait(CGThing):
    def __init__(self, descriptor):
        CGThing.__init__(self)
        proxy = "None"
        if descriptor.proxy:
            proxy = "Some(PrototypeList::Proxies::%s)" % descriptor.name
        final = "true"
        if descriptor.interface.getUserData("hasConcreteDescendant", False):
            final = "false"
        code = """
impl MagicDOMClass for %s {
    const PROTO_ID: u32 = PrototypeList::ID::%s as u32;
    const PROTO_DEPTH: usize = %s;
    const PROXIED: Option<PrototypeList::Proxies> = %s;
    const FINAL: bool = %s;
    fn create_proto(cx: *mut JSContext, global: HandleObject) -> *mut JSObject {
        let mut proto = RootedObject::new(cx, ptr::null_mut());
        CreateInterfaceObjects(cx, global, global, proto.handle_mut());
        proto.ptr
    }
    fn get_jsclass() -> *const js::jsapi::Class {
        if <Self as SlotCount>::NEED_FINALIZE {
            &Class_finalize.base
        } else {
            &Class.base
        }
...
class MethodDefiner(PropertyDefiner):
    ...
    def __init__(self, descriptor, name, static):
        ...
        self.regular = []
        for m in methods:
            entry = {"name": m.identifier.name,
                     "methodInfo": not m.isStatic(),
                     "length": methodLength(m),
                     "flags": "JSPROP_ENUMERATE"}
            if m.getExtendedAttribute("SelfHosted"):
                entry["selfHostedName"] = descriptor.interface.identifier.name + "_fn_" + m.identifier.name
                entry["methodInfo"] = False
...
class AttrDefiner(PropertyDefiner):
    ...
    def generateArray(self, array, name):
        ...
        def flags(attr):
            if attr.getExtendedAttribute("SelfHosted"):
                if attr.readonly:
                    return "JSPROP_SHARED | JSPROP_ENUMERATE | JSPROP_GETTER"
                else:
                    return "JSPROP_SHARED | JSPROP_ENUMERATE | JSPROP_GETTER | JSPROP_SETTER"
            else:
        ...
        def getter(attr):
            if attr.getExtendedAttribute("SelfHosted"):
                accessor = "None"
                jitinfo = str_to_const_array(self.descriptor.interface.identifier.name + "_get_" + attr.identifier.name) + " as *const u8 as *const JSJitInfo"
            elif self.static:
                accessor = 'Some(get_' + self.descriptor.internalNameFor(attr.identifier.name) + ')'
                jitinfo = "0 as *const JSJitInfo"
            else:
        ...
        def setter(attr):
            ...
            if attr.getExtendedAttribute("SelfHosted"):
                accessor = "None"
                jitinfo = str_to_const_array(self.descriptor.interface.identifier.name + "_set_" + attr.identifier.name) + " as *const u8 as *const JSJitInfo"
            elif self.static:
                accessor = 'Some(set_' + self.descriptor.internalNameFor(attr.identifier.name) + ')'
                jitinfo = "0 as *const JSJitInfo"
            else:
                if attr.hasLenientThis():
...
def DOMClass(descriptor):
        ...
        proxy_slot = "None"
        if descriptor.proxy:
            proxy_slot = "Some(%s::SLOT_COUNT)" % descriptor.name
        return """\
DOMClass {
    interface_chain: [ %s ],
...
}""" % (prototypeChainString, proxy_slot, DOMClassTypeId(descriptor), heapSizeOf)
...
class CGDOMJSClass(CGThing):
    ...
    def define(self):
        return self.inner_define(True) + "\n" + self.inner_define(False)

    def inner_define(self, finalize):
        className = 'Class'
        finalizeHook = 'None'
        if finalize:
            className = 'Class_finalize'
            finalizeHook = 'Some(finalize_hook::<%s>)' % self.descriptor.name
        traceHook = 'Some(trace_hook::<%s>)' % self.descriptor.name
        slots = "%s::SLOT_COUNT as u32" % self.descriptor.name
        ...
        if self.descriptor.isGlobal():
            ...
            flags = "0"
        return """\
static %s: DOMJSClass = DOMJSClass {
    base: js::jsapi::Class {
        name: %s as *const u8 as *const libc::c_char,
...
class CGCreateInterfaceObjectsMethod(CGAbstractMethod):
    ...
    def definition_body(self):
        ...
        if self.descriptor.concrete:
            domClass = "unsafe { Some(&(&*(%s::get_jsclass() as *const DOMJSClass)).dom_class) }" % self.descriptor.name
        else:
            domClass = "None"
...
CreateProxyHandler(&traps, &(&*(%s::get_jsclass() as *const DOMJSClass)).dom_class as *const _ as *const _)\
""" % (customDefineProperty, customDelete, self.descriptor.name)
    ...
    def __init__(self, errorResult, arguments, argsPre, returnType,
        ...
        if isFallible:
            if static:
                ...
                glob = "        let global = global_object_for_dom_object(%s);\n" % object
...
class CGSpecializedMethod(CGAbstractExternMethod):
    ...
    def __init__(self, descriptor, method):
        ...
        args = [Argument('*mut JSContext', 'cx'), Argument('HandleObject', 'obj'),
    ...
    def definition_body(self):
        ...
        return CGWrapper(CGMethodCall([], nativeName, self.method.isStatic(),
                         ...
                         pre="let this = &*(obj.ptr as *const %s);\n"
                             "let args = &*args;\n"
...
class CGProxyUnwrap(CGAbstractMethod):
    def __init__(self, descriptor):
        ...
        CGAbstractMethod.__init__(self, descriptor, "UnwrapProxy",
                                  'Root<%s>' % descriptor.concreteType, args,
...
Root::new(NonZero::new(GetProxyExtra(*obj.ptr, 0).to_object()))
""")
...
class CGDescriptor(CGThing):
    def __init__(self, descriptor):
        ...
        if descriptor.concrete:
            ...
            cgThings.append(CGMagicClassTrait(descriptor))
...
class CGBindingRoot(CGThing):
    ...
    def __init__(self, config, prefix, webIDLFile):
        ...
        curr = CGImports(curr, descriptors + callbackDescriptors, mainCallbacks, [
            ...
            'js::{JSPROP_ENUMERATE, JSPROP_SHARED, JSPROP_GETTER, JSPROP_SETTER}',
            ...
            'js::glue::{GetProxyExtra, NewProxyObject, ProxyTraps}',
            ...
            'dom::bindings::global::global_object_for_dom_object',
            ...
            'dom::bindings::magic::{MagicDOMClass, SlotCount, finalize_hook}',
...
class GlobalGenRoots():
    ...
    def InheritTypes(config):
        ...
        imports = [CGGeneric("use dom::types::*;\n"),
                   ...
                   CGGeneric("use dom::bindings::magic::MagicDOMClass;\n"),
